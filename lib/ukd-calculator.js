class UKDCalculator {
    ...
    expectedScore(playerRating, opponentRating) {
        ...
        if (opponentRating === 0) {
            ratingDiff = 350; // Assume 350 point difference for unrated opponents
        } else {
            ratingDiff = Math.abs(playerRating - opponentRating);
        }
        
        const [highProb, lowProb] = this.lookupRatingDiff(ratingDiff);
        
        // Return probability for the higher-rated player
        // For unrated opponents (rating = 0), always assume player is higher rated
        if (opponentRating === 0 || playerRating >= opponentRating) {
    ...
    // Extract match data from HTML using cheerio
    extractMatchData($) {
        const matches = [];
        ...
        $('table.CRs1').each((tableIndex, table) => {
            ...
            rows.each((rowIndex, row) => {
                ...
                if (cells.length >= 2) {
                    ...
                    // Look for rating in various formats (Turkish and English)
                    if (label.toLowerCase().includes('rating') || 
                        ...
                        if (rating >= 1000 && rating <= 3000 && !playerRating) {
                            playerRating = rating;
                            console.log('UKD Calculator: Found player rating:', rating, 'from label:', label);
                        }
                    }
                }
            });
        ...
        $('table.CRs1').each((tableIndex, table) => {
            ...
            if (headerText.includes('Tur') || headerText.includes('Rd.')) {
                if (headerText.includes('Sonuç') || headerText.includes('Res.')) {
                    console.log('UKD Calculator: Found results table');
                    
                    // Find the column indices for rating and result
                    ...
                    headerCells.each((i, cell) => {
                        ...
                        // Look for rating columns in order of preference
                        if (headerText === 'ukd') {
                            ratingColumnIndex = i;
                    ...
                    console.log('UKD Calculator: Rating column index:', ratingColumnIndex, 'Result column index:', resultColumnIndex);
                    ...
                    rows.slice(1).each((rowIndex, row) => {
                        ...
                        if (cells.length > Math.max(ratingColumnIndex, resultColumnIndex) && 
                            ...
                            // Extract opponent rating from the correct column
                            if (ratingColumnIndex >= 0 && cells[ratingColumnIndex]) {
                                ...
                                // Keep rating = 0 as 0 (unrated) for special handling
                                if (rating === 0) {
                                    opponentRating = 0;
                                    console.log('UKD Calculator: Found unrated opponent (rating = 0)');
                                } else if (rating >= 1000 && rating <= 3000) {
                                    opponentRating = rating;
                                }
                            }
                            
                            // Extract result from the correct column
                            if (resultColumnIndex >= 0 && cells[resultColumnIndex]) {
                                ...
                                // Skip forfeit cases where no actual game was played
                                if (cellText.includes('K') || cellText.includes('k')) {
                                    console.log('UKD Calculator: Skipping forfeit result:', cellText);
                                    skippedRounds.add(rowIndex);
                                    return; // Continue to next iteration
                                }
                                
                                // The result is in a nested table structure
                                ...
                                if (resultTable.length > 0) {
                                    ...
                                    // Try extracting directly from cell if no nested table
                                    if (cellText === '1' || cellText === '1.0') {
                                        result = 1;
                                    } else if (cellText === '0' || cellText === '0.0') {
                                        result = 0;
                                    } else if (cellText === '½' || cellText === '0.5') {
                                        result = 0.5;
                                    }
                                }
                            }
                            ...
                            if (opponentRating !== null && result !== null) {
                                matches.push([opponentRating, result]);
                                console.log('UKD Calculator: Found match - Opponent rating:', opponentRating, 'Result:', result);
                            }
                        }
                    });
                }
        ...
        console.log('UKD Calculator: Final results - Player rating:', playerRating, 'Matches:', matches.length);
        return { playerRating, matches, skippedRounds };
    }
}

module.exports = UKDCalculator;