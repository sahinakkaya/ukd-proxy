    ...
    try {
        ...
        $('a[href*="chess-results.com"]').each((index, element) => {
            ...
            if (href && text && href.includes('chess-results.com')) {
                ...
                // If tournament name is still the generic link text, look for h2 with actual tournament name
                if (tournamentName === text && text.includes('Eşlendirme')) {
                    // Look for the nearest h2 tag with "TURNUVA" or similar
                    let $h2 = $link.prevAll('h2').first();
                    if (!$h2.length) {
                        // Look in parent containers
                        $h2 = $link.parents().prevAll('h2').first();
                        if (!$h2.length) {
                            // Look for any h2 in the parent structure
                            $h2 = $link.closest('body').find('h2').filter(function() {
                                const h2Text = $(this).text().toUpperCase();
                                return h2Text.includes('TURNUVA') || h2Text.includes('SATRANÇ');
                            }).last();
                        }
                    }
                    ...
                    if ($h2.length) {
                        ...
                        if (h2Text.length > 5 && (h2Text.toUpperCase().includes('TURNUVA') || h2Text.toUpperCase().includes('SATRANÇ'))) {
