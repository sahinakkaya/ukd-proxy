...
async function addUKDCalculations(html, originalUrl) {
    try {
        ...
        const { playerRating, matches, ongoingMatches, skippedRounds } = calculator.extractMatchData($);
        
        // If no valid data found, return original HTML
        if (!playerRating || (matches.length === 0 && (!ongoingMatches || ongoingMatches.length === 0))) {
            console.log('No valid UKD data found, returning original HTML');
            return html;
        }
        
        // Calculate UKD changes for completed matches
        const ukdData = calculator.calculateUKDChange(playerRating, matches);
        ukdData.skippedRounds = skippedRounds;
        
        // Calculate potential changes for ongoing matches
        const potentialChanges = ongoingMatches && ongoingMatches.length > 0 
            ? calculator.calculatePotentialChanges(playerRating, ongoingMatches) 
            : [];
        ukdData.potentialChanges = potentialChanges;
        
        console.log(`UKD Calculator: Processing ${matches.length} completed matches and ${ongoingMatches ? ongoingMatches.length : 0} ongoing matches for rating ${playerRating}`);
    ...
    $('table.CRs1').each((tableIndex, table) => {
        ...
        if ((headerText.includes('Tur') || headerText.includes('Rd.')) && 
            ...
            if (headerCells.length > 0 && !headerRow.find('.ukd-header').length) {
                const expectedHeader = $('<th class="CRc"></th>');
                const changeHeader = $('<th class="CRc"></th>');
            ...
            rows.slice(1).each((rowIndex, row) => {
                ...
                if (cells.length >= 10 && !$row.find('.ukd-data').length) {
                    ...
                    if (ukdData.skippedRounds && ukdData.skippedRounds.has(rowIndex)) {
                        ...
                        // Add actual calculation data for completed matches
                        const calc = ukdData.calculations[calcIndex];
                        
                        const expectedCell = $('<td class="ukd-data"></td>');
                        const changeCell = $('<td class="ukd-data"></td>');
                        
                        expectedCell.text(calc.expected.toFixed(2));
                        ...
                        // Add potential outcomes for ongoing matches
                        const potential = ukdData.potentialChanges[potentialIndex];
                        
                        const expectedCell = $('<td class="ukd-data"></td>');
                        const changeCell = $('<td class="ukd-data ukd-potential"></td>');
                        
                        expectedCell.text(potential.win.expected.toFixed(2));
                        
                        // Create single row for potential outcomes
                        ...
                        const potentialHtml = `
                            <div class="potential-outcomes-single" title="${isEnglish ? 'Win / Draw / Loss' : 'Galibiyet / Beraberlik / Mağlubiyet'}">
        ...
        <div id="ukd-calculator-display" class="ukd-calculator-summary">
            ...
            <div class="ukd-summary-content">
                ...
                <div class="ukd-summary-item">
                    ...
                    <span class="ukd-value">${
                        ukdData.potentialChanges && ukdData.potentialChanges.length > 0 
                            ? `${ukdData.calculations.length}/${ukdData.calculations.length + ukdData.potentialChanges.length}`
...
function addUKDStyles($) {
    ...
    const styles = `
        <style>
            ...
            /* Potential outcomes styling */
            .potential-outcomes-single {
                display: flex;
                align-items: center;
                font-size: 11px;
                font-weight: bold;
            ...
            .potential-outcomes-single .outcome {
                padding: 1px 3px;
                border-radius: 2px;
                margin: 0 1px;
            }
            .potential-outcomes-single .outcome.win {
                background-color: #d4edda;
                color: #155724;
            }
            .potential-outcomes-single .outcome.draw {
                background-color: #fff3cd;
                color: #856404;
            }
            .potential-outcomes-single .outcome.loss {
                background-color: #f8d7da;
                color: #721c24;
            }
            .potential-outcomes-single .separator {
                color: #6c757d;
                font-weight: normal;
                margin: 0 2px;
            }
